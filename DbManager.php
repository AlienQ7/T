<?php
// =========================================================
// DbManager.php (V2.2 - CRITICAL FIX: Hash size and Point Columns)
// =========================================================
...
class DbManager {
    ...
    private function initializeDatabase() {
        ...
        $this->db->exec("
            CREATE TABLE IF NOT EXISTS users (
                ...
                hash VARCHAR(255) NOT NULL, -- CRITICAL: Ensures the full password hash is stored
                user_objective TEXT DEFAULT '',
                rank TEXT DEFAULT '',
                sp_points INTEGER DEFAULT 0,
                task_points INTEGER DEFAULT 0,
                claimed_task_points INTEGER DEFAULT 0, -- The missing column for V1.3 fix
        ...
        // Simple ALTER TABLE attempt for existing databases (best effort)
        try {
            $this->db->exec("ALTER TABLE users ADD COLUMN claimed_task_points INTEGER DEFAULT 0");
        } catch (Exception $e) {
        ...
        $this->db->exec("
            CREATE TABLE IF NOT EXISTS sessions (
                token VARCHAR(64) PRIMARY KEY, -- Increased size for secure tokens
    ...
    // =================================================
    // (Functions getUserData, saveUserData, saveNewUser etc. rely on the correct schema above)
    // (The rest of the DbManager functions are assumed to be the same as previously provided and correct)
    ...
    public function saveUserData($username, $userData) {
        // CRITICAL: Added 'claimed_task_points' to the list of fields to save
        $fields = [
            'rank', 'sp_points', 'task_points', 'claimed_task_points', 'failed_points', 
